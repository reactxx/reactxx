...
export const taskDefaultCreator = (functionName:string) => (root: Ast.Ast) => {
    removeClassNamesImport(root)
    selectClassNamesFromProps(root, functionName)
    adjustHtmlClassName(root, functionName)
}
...
const adjustHtmlClassName = (root: Ast.Ast, functionName: string) => {
...
const selectClassNamesFromProps = (root: Ast.Ast, functionName: string) => {
    ...
    (place as Array<any>).splice(0, 0, selectFromObject('classNames'), selectFromObject('classNamesStr'))
