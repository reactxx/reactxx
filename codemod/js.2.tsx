...
const withStyles = (stylesOrCreator, options: any = {}) => Component => {
  const {
    withTheme = false,
    flip = null,
    name,
  ...
  const stylesCreator: any = getStylesCreator(stylesOrCreator);
  ...
  return (WithStyles as React.ComponentClass<any>);
};
...
export type WithStyles<T extends string | StyleRules | StyleRulesCallback = string> = Partial<WithTheme> & {
  classes: ClassNameMap<T extends string ? T : T extends StyleRulesCallback<infer K> ? K : T extends StyleRules<infer K> ? K : never>;
};
