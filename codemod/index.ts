...
import ButtonBase from './patches/ButtonBase/ButtonBase'
...
Tasks.adjustHtmlClassName(Parser.parseFile('D:/reactxx/codemod/patch-sources/Button/Button.js'), 'Button')
//Prepare.prepareAstDump(Button[0].path)

const x = () => {

    const node = ts.createNode(ts.SyntaxKind.Parameter)
    ...
    const srcPath = 'd:/reactxx/codemod/flow/'
    const prepareForTransformDir = 'd:/reactxx/codemod/patch-sources/'

    // const transform = () => {
    //     patches.forEach(patch => {
    //         patch.forEach(p => p.transform(''))
    //     })
    // }

    // const prepareForTransform = () => {
    //     patches.forEach(patch => {
    //         patch.forEach(p => {
    ...
    const getNodeFromPath = (root, path: string) => {
        if (!path) return null
    ...
    const navigateAll = (ast, path: string) => {
        const res = []
        let x = getNodeFromPath(ast, path)
        res.push(x)
        while (x.$parentPath) {
            x = getNodeFromPath(ast, x.$parentPath)
            res.push(x)
        }
        return res
    }

    const removeTemporaryFields = root => {
        for (const p in root) {
    ...
    const removeNode = (root, path: string) => {
        if (!path) return null
        const parts = path.split('/')
        parts.forEach((part, idx) => {
    ...
    removeNode(ast, importClassNames[0].$path) //, '*** REACTXX PATCH: remove classNames import')
