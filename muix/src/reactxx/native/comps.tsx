...
const anyView = (isAnim: boolean) => (props => {
  ...
  const { style, classes, className_, mergeRulesetWithOverrides, theme, onPress, onLongPress, onPressIn, onPressOut, animations, mediaq, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, className_, style) as ReactN.ViewStyle
...
const anyText = (isAnim: boolean) => (props => {
  ...
  const { style, classes, className_, mergeRulesetWithOverrides, theme, animations, mediaq, url, onPress, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, props.numberOfLines === 1 && classes.singleLineStyle, className_, style) as ReactN.TextStyle
  //Link to URL
...
const anyScrollView = (isAnim: boolean) => (props => {
  ...
  const { style, classes, className_, mergeRulesetWithOverrides, theme, animations, mediaq, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, className_, style) as ReactN.ScrollViewStyle
...
const anyIcon = (isAnim: boolean) => (props => {
  ...
  const { style, classes, className_, mergeRulesetWithOverrides, theme, animations, data, children, mediaq, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, className_, style) as ReactN.TextStyle
