...
const view: TBasic.CodeSFCWeb<TComps.ViewShape> = props => {
  ...
  const rootStyle = mergeRulesetWithOverrides(classes.root, className_)
  return <div className={rulesetsToClassNames(rootStyle)} style={style} {...rest} />
}

const icon: TBasic.CodeSFCWeb<TComps.IconShape> = props => {
  const { style, classes, className_, data, mergeRulesetWithOverrides, theme, animations, viewBox, children/*this children*/, mediaq, url, onClick, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, onClick && classes.pressable, className_)
...
const text: TBasic.CodeSFCWeb<TComps.TextShape> = props => {
  const { style, classes, className_, numberOfLines, mergeRulesetWithOverrides, theme, animations, mediaq, url, onClick, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, onClick && classes.pressable, numberOfLines === 1 && classes.singleLineStyle, className_)
...
const scrollView: TBasic.CodeSFCWeb<TComps.ScrollViewShape> = props => {
  const { style, classes, className_, horizontal, mergeRulesetWithOverrides, theme, animations, children, mediaq, ...rest } = props
  const rootStyle = mergeRulesetWithOverrides(classes.root, horizontal && classes.rootHorizontal, className_)
