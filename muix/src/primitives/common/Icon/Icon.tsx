...
const icon: Prim5s.CodeSFC<MuixIcon.Shape> = props => {
  const { classes, theme, flip, getRulesetWithSideEffect, style, children, color = 'inherit', className, animations, ...rest } = props
  const childs = React.Children.toArray(children)
  warning(childs.length == 1 && typeof childs[0] === 'string', 'single child as string expected')
  const data = childs[0] as MuixIcons
  const iconClass = getRulesetWithSideEffect(classes.root, classes[`color${capitalize(color)}`], className)
