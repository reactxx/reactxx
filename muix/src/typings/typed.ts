...
export type RulesetIds = $W | $T | $V | $I | TTyped.CommonIds
...
export type TAllowedInput<R extends RulesetIds> = TTyped.RulesetType<R> | TAllowed<R>
...
export interface Utils<P extends {}, Theme extends {}> {
  ...
  $native: <R extends TTyped.CommonIds>(...r: TAllowedInput<TTyped.TNative<R>>[]) => R
...
namespace TTyped {
  ...
  export type RulesetType<R extends RulesetIds> =
  ...
  export type TNative<R extends T | V | I> =
    R extends T ? $T :
    R extends V ? $V :
    R extends I ? $I :
    never
