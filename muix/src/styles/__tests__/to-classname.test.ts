...
describe("TO CLASSNAMES", () => {
  ...
  const query = (opened: boolean, ...ruleset: TTyped.RulesetSimple[]) => dump(toClassNamesWithQuery(sheetQuery(opened), ...ruleset))
  ...
  const doTest = (isWeb: boolean) => {
    ...
    it("02: null, {}, undefined", () => dump(toClassNamesWithQuery(null, undefined)))
    ...
    it("08: static if and ifelse", () => {
      const inRender = (disabled: boolean) =>
        dump(toClassNamesWithQuery(sheetQuery(false),
