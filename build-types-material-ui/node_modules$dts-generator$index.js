//*****************************************************
//
//  This file replaces original dts-generator's D:\reactxx\build-types-material-ui\node_modules\dts-generator\index.js file
//
//*****************************************************
...
(function (factory) {
  ...
  //LM CHANGE
  function processTree(sourceFile, replacer) { //LM
    var code = '';
    function visit(node) {
      //code += '\r\n/******* ' + node.kind + ':' + sourceFile.text.slice(node.pos, node.end) + '************/\r\n'
      if (node.kind === ts.SyntaxKind.TypeAliasDeclaration || node.kind === ts.SyntaxKind.InterfaceDeclaration || node.kind === ts.SyntaxKind.ClassDeclaration || node.kind === ts.SyntaxKind.ModuleDeclaration) {
        code += sourceFile.text.slice(node.pos, node.end);
        code += '\r\n';
  ...
  function generate(options) {
    ...
    filenames = filenames.map(function (f) { return f.split('\\').join('/') }) // LM
    ...
    return new Promise(function (resolve, reject) {
      ...
      output.write('/**' + eol)
      output.write(' * This code was generated by a tool from https://github.com/mui-org/material-ui.' + eol)
      output.write(' * Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.' + eol)
      output.write('*/' + eol)
      //output.write("import { TransitionGroup } from 'react-transition-group'" + eol)
      //output.write("import { IPopperProps } from 'react-popper'" + eol)
      //output.write("import { StyleSheet } from 'jss'" + eol)
      output.write('declare namespace Muix {' + eol); //LM
      ...
      program.getSourceFiles().some(function (sourceFile) {
        //if (sourceFile.fileName.indexOf('index') >= 0) return; // LM
        //if (options.mask && sourceFile.fileName.indexOf(options.mask) < 0) return;
    ...
    function writeDeclaration(declarationFile, isOutput) {
      ...
      //output.write(JSON.stringify(declarationFile,null,2));  //LM 
      if (declarationFile.externalModuleIndicator) {
        ...
        // LM output.write('declare module \'' + resolvedModuleId + '\' {' + eol + indent);
        output.write('\r\n\r\n//************* ' + resolvedModuleId + '\r\n'); //LM
        ...
        output.write(content.replace(nonEmptyLineStart, '$&' + indent).replace(/export /g, ''));
        // LM output.write(eol + '}' + eol);
      }
      else {
