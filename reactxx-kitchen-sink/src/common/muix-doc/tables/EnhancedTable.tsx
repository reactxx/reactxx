...
import TablePagination from 'reactxx-muix/current/TablePagination/TablePagination';
import TableRow from 'reactxx-muix/current/TableRow/TableRow';
import TableSortLabel from 'reactxx-muix/current/TableSortLabel/TableSortLabel';
...
import FilterListIcon from 'reactxx-icons/FilterList';
import { lighten } from 'reactxx-mui-web/styles/colorManipulator';
let counter = 0;
  ...
  render() {
    ...
    return <TableHead>
        <TableRow>
          ...
          return <TableCell key={column.id} numeric={column.numeric} padding={column.disablePadding ? 'none' : 'default'} sortDirection={orderBy === column.id ? order : false}>
...
let EnhancedTableToolbar: any = props => {
  ...
  return <Toolbar className={classNames(classes.root, {
    [classes.highlight]: numSelected > 0
      ...
      <div className={classNamesStr(classes.title)}>
        {numSelected > 0 ? <Typography color="inherit" variant="subheading">
            {numSelected} selected
          </Typography> : <Typography variant="title" id="tableTitle">
            Nutrition
          </Typography>}
      </div>
      <div className={classNamesStr(classes.spacer)} />
      <div className={classNamesStr(classes.actions)}>
...
EnhancedTableToolbar = withStylesCreator((toolbarStyles as any), EnhancedTableToolbar)();
        ...
        <div className={classNamesStr(classes.tableWrapper)}>
          <Table className={classNames(classes.table)} aria-labelledby="tableTitle">
            ...
            <TableBody>
              ...
              return <TableRow hover onClick={event => this.handleClick(event, n.id)} role="checkbox" aria-checked={isSelected} tabIndex={-1} key={n.id} selected={isSelected}>
                      <TableCell padding="checkbox">
...
export default withStylesCreator((styles as any), EnhancedTable)();